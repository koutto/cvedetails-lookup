...
def warning(string):
    ...
    print(
        colorize("[!] ", color="dark_orange", attrs="bold")
...
# ------------------------------------------------------------------------------
info(
    'Looking for "{vendor}{delim}{product} {version}" in cvedetails.com '
    "database...".format(
...
if "List of cve security vulnerabilities related to this exact version" not in resp:
    ...
    if "No matches" not in resp:
        ...
        if args.version in versions_results:
            ...
            version = args.version
            success(
                "Exact match found in the database (in {} entries, "
    ...
    if not version_found:
        warning(
            "No exact match for this product/version. "
            "Checking for CVE in newer versions..."
        )
        ...
        if "No matches" in resp:
            error(
                'The product "{vendor}{delim}{product}" is not referenced in '
                "cvedetails.com database !".format(
                    vendor=args.vendor or "",
                    delim=" " if args.vendor else "",
                    product=args.product,
                )
        ...
        # Check if there are referenced newer versions and take the closest one to the 
        # targeted version.
        # Looping around targeted version till a newer version is found because 
        # cvedetails limits to 100 results per search.
        # E.g. searching for "Nginx 0.8" will not work, for "Nginx" without version 
        # will only display the first 100 results which are versions older than 0.8, 
        # but searching for "Nginx 0.8%" will allow to find newer version.
        i = len(args.version)
        ...
        while i >= 0:
            ...
            # If table with matching versions displayed
                ...
                # Need to handle several ids for a given version because cvedetails can 
                # have several ids for different languages, editions, updates for a 
                # given version number
                versions_results = parse_html_table_versions(resp)

                version = get_closest_superior_version(
                    args.version, versions_results.keys()
                )
...
#
info(
    "IDs summary: Vendor={vendor} [{vendor_id}] | Product={product} "
